...
snapshot_handles = [MR.PickleVecHandle('vec%d.pkl' % i, scale=quad_weights[i])
...
if parallel.is_rank_zero():
    for i, snap in enumerate(snapshot_handles):
        snap.put(N.random.random(num_elements))
parallel.barrier()

# Compute POD modes
my_POD = MR.POD(N.vdot)
my_POD.compute_decomp(snapshot_handles)
...
IP_mat = vec_space.compute_symmetric_inner_product_mat(mode_handles)
