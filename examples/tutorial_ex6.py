...
direct_snapshot_handles = [CustomVecHandle('direct_snap%d.pkl' % i, scale=N.pi)
    for i in range(10)]
adjoint_snapshot_handles = [CustomVecHandle('adjoint_snap%d.pkl' % i, scale=N.pi)
    for i in range(10)]
...
if parallel.is_rank_zero():
    for snap in direct_snapshot_handles + adjoint_snapshot_handles:
        snap.put(CustomVector([x, y, z], N.random.random((nx, ny, nz))))
parallel.barrier()

# Compute balanced POD
my_BPOD = MR.BPOD(inner_product)
my_BPOD.sanity_check(direct_snapshot_handles[0])
...
direct_mode_handles = [CustomVecHandle('direct_mode%d.pkl'%i) for i in mode_nums] 
adjoint_mode_handles = [CustomVecHandle('adjoint_mode%d.pkl'%i) for i in mode_nums]
