...
class TestDMDArraysFunctions(unittest.TestCase):
    ...
    def test_all(self):
        ...
        for weights in weights_list:
            ...
            np.testing.assert_allclose(ritz_vals, ritz_vals_true, rtol=tol)
            np.testing.assert_allclose(build_coeffs, build_coeffs_true, rtol=tol)
            np.testing.assert_allclose(modes, ritz_vecs_true[:, mode_indices], 
                rtol=tol)
...
class TestDMDHandles(unittest.TestCase):
    ...
    def test_gets_puts(self):
        ...
        if not os.path.isdir(test_dir) and _parallel.is_rank_zero():
            os.mkdir(test_dir)
        ritz_vals = _parallel.call_and_bcast(np.random.random, 5)
        mode_norms = _parallel.call_and_bcast(np.random.random, 5)
        build_coeffs = _parallel.call_and_bcast(np.random.random, (10,10))
        ...
        np.testing.assert_allclose(DMD_load.ritz_vals, ritz_vals)
        np.testing.assert_allclose(DMD_load.build_coeffs, build_coeffs)
        np.testing.assert_allclose(DMD_load.mode_norms, mode_norms)
    ...
    def _helper_check_decomp(self, ritz_vals, build_coeffs, mode_norms, 
        ...
        np.testing.assert_allclose(self.my_DMD.ritz_vals, 
            ritz_vals, rtol=tol)
        np.testing.assert_allclose(self.my_DMD.build_coeffs, 
            build_coeffs, rtol=tol)
        np.testing.assert_allclose(self.my_DMD.mode_norms, 
            mode_norms, rtol=tol)

        # Test that matrices were correctly returned
        np.testing.assert_allclose(ritz_vals_returned, 
            ritz_vals, rtol=tol)
        np.testing.assert_allclose(build_coeffs_returned, 
            build_coeffs, rtol=tol)
        np.testing.assert_allclose(mode_norms_returned, 
            mode_norms, rtol=tol)
    ...
    def _helper_check_modes(self, ritz_vecs, build_coeffs, vec_handles):
        ...
        mode_indices = list(range(build_coeffs.shape[1]))
        ...
        # computation
        mode_array = np.array(np.zeros((self.num_states, build_coeffs.shape[1])), 
            dtype=complex)
        for i in range(build_coeffs.shape[1]):
            mode_array[:, i] = V.VecHandlePickle(mode_path % i).get()
        np.testing.assert_allclose(mode_array, ritz_vecs, rtol=tol)
