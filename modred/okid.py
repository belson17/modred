...
def OKID(inputs, outputs, num_Markovs):
    ...
    if num_samples != num_samples_outputs:
        raise ValueError(
        'Number of samples in input and output differ, %d != %d'
        % (num_samples, num_samples_outputs))
    ...
    Markov_aug = np.linalg.lstsq(V.conj().T, outputs.T)[0].conj().T
    ...
    for Markov_num in range(num_Markovs):
        ...
        Markov_aug_output.append(
            -Markov_aug_noD[
                :,
                Markov_num * num_inouts + num_inputs:
    ...
    for Markov_num in range(1, num_Markovs):
        ...
        for i in range(1, Markov_num + 1):
            summation_term += Markov_aug_output[i].dot(
                Markov_est[Markov_num- i , :, :])
        Markov_est[Markov_num] = (
            Markov_aug_input[Markov_num] - summation_term)
