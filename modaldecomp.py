...
class ModalDecomp(object):
    ...
    def _compute_symmetric_inner_product_chunk(self, fieldPaths):
        """ Computes a symmetric matrix of inner products.  E.g. in POD this is
        ...
        numRows = len(fieldPaths)
        numCols = numRows

        if self.verbose:
            # Print after this many rows are computed
            printAfterNumRows = (numRows / 5) + 1 

        # For a symmetric block, if all rows can be loaded, don't need to load
        # any columns
        if numRows <= self.maxFieldsPerProc:
            numColsPerChunk = 0 
        else:
            numColsPerChunk = 1 
        numRowsPerChunk = self.maxFieldsPerProc - numColsPerChunk         
        ...
        for startRowIndex in range(0, numRows, numRowsPerChunk):
            ...
            rowFields = []
            for rowPath in fieldPaths[startRowIndex:endRowIndex]:
            ...
            # row index).  Only do this if not all rows can be loaded.
            if numColsPerChunk != 0:
                for startColIndex in range(endRowIndex, numCols, 
                    numColsPerChunk):
                    endColIndex = min(numCols, startColIndex + numColsPerChunk)
                    colFields = []
                    for colPath in fieldPaths[startColIndex:endColIndex]:
            ...
            # Print a progress report if a certain number of rows have been
            # computed
            if self.verbose and (endRowIndex % printAfterNumRows == 0 or \
                endRowIndex==numRows): 
                # Triangular part
                numCompletedIPs = 0.5 * endRowIndex * (endRowIndex + 1)

                # Rectangular part
                numCompletedIPs += endRowIndex * (numCols - endRowIndex)
                
                percentCompletedIPs = 100 * numCompletedIPs / (0.5 * numRows * (
                    numRows + 1))

                print >> sys.stderr, ('Processor %d completed symmIPMat[' +\
                    ':%d, :] of symmIPMat[:%d, :]') % (self.mpi.getRank(), 
                    endRowIndex, numRows)  
                print >> sys.stderr, ('    %.1f%% of inner products ' +\
                    'completed on this processor') % percentCompletedIPs

        # Now that upper triangular part is computed, symmetrize
        # Pull out upper triangular part of matrix (minus diagonal), 
        # transpose, and add to original matrix
        innerProductMatChunk += N.triu(innerProductMatChunk, 1).T
        return innerProductMatChunk
