...
class TestDMD(unittest.TestCase):
    ...
    def generate_data_set(self):
        ...
        if parallel.isDistributed():
            self.snapPathList = parallel.comm.bcast(self.snapPathList, 
                root=0)
    ...
    def test_init(self):
        ...
        dataMembersDefault = {'save_mat': util.save_mat_text, 'load_mat': util.\
            load_mat_text, 'pod': None, 'parallel': parallel_mod.parallelInstance,\
            'verbose': False, 'fieldOperations': FieldOperations(load_field=None, 
            save_field=None, inner_product=None, maxFieldsPerNode=2,
            verbose=False)}
    ...
    def test_compute_decomp(self):
        ...
        if parallel.isDistributed():
            ritzValsLoaded = parallel.comm.bcast(ritzValsLoaded,root=0)
            buildCoeffLoaded = parallel.comm.bcast(buildCoeffLoaded,root=0)
            modeNormsLoaded = parallel.comm.bcast(modeNormsLoaded,
                root=0)
    ...
    def test_compute_modes(self):
        ...
        if parallel.isRankZero():
            ...
            modeMat = None
        if parallel.isDistributed():
            modeMat = parallel.comm.bcast(modeMat,root=0)
