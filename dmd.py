...
class DMD(object):
    ...
    def __init__(self, load_field=None, save_field=None, save_mat=util.\
        save_mat_text, inner_product=None, maxFields=None,
        pod=None, verbose=True):
        """
        DMD constructor. Uses FieldOperations instance and POD instanced
        for low-level operations and parallel processing.
        """
        ...
        # There is no load_mat in the constructor?? JON??
    ...
    def compute_decomp(self, ritzValsPath=None, modeNormsPath=None, 
        ...
        if self.pod is None:
            self.pod = POD(load_field=self.load_field, inner_product=self.\
                inner_product, snapPaths=self.snapPaths[:-1], maxFields=\
                self.maxFields, numNodes=self.numNodes, verbose=self.\
                verbose)
