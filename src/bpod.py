...
def compute_BPOD_arrays(direct_vec_array, adjoint_vec_array, direct_mode_indices,
    adjoint_mode_indices, inner_product_weights=None, return_all=False):
    """Computes BPOD modes as columns in 2D arrays.
        
    Args:
        ``direct_vec_array``: 2D array with direct vecs as columns (:math:`X`).
    
        ``adjoint_vec_array``: 2D array with adjoint vecs as columns (:math:`Y`).
    ...
    Returns:
        ``direct_modes``: 2D array with direct modes as columns.
        
        ``adjoint_modes``: 2D array with adjoint modes as columns.
        ...
        ``R_sing_vecs``: Matrix of right singular vectors of Hankel mat (:math:`V`).

        ``Hankel_mat``: Hankel matrix (:math:`Y^*X`).
    ...
    direct_vec_array = util.make_2D_array(direct_vec_array)
    adjoint_vec_array = util.make_2D_array(adjoint_vec_array)
    
    Hankel_mat = vec_space.compute_inner_product_mat(adjoint_vec_array, 
        direct_vec_array)
    L_sing_vecs, sing_vals, R_sing_vecs = util.svd(Hankel_mat)
    
    sing_vals = N.squeeze(N.array(sing_vals))
    direct_build_coeff_mat = N.dot(R_sing_vecs, N.diag(sing_vals**-0.5))
    direct_mode_array = vec_space.lin_combine(direct_vec_array, 
        direct_build_coeff_mat, coeff_mat_col_indices=direct_mode_indices)
     
    adjoint_build_coeff_mat = N.dot(L_sing_vecs, N.diag(sing_vals**-0.5))
    adjoint_mode_array = vec_space.lin_combine(adjoint_vec_array,
