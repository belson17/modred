...
class BPODROM(object):
    ...
    def __init__(self, dt=None, inner_product=None, put_mat=util.save_mat_text,
      get_vec=None, put_vec=None, verbose=True, num_modes=None, max_vecs_per_node=2):
          self.dt = dt
    ...
    def compute_mode_derivs(self, mode_sources, mode_dt_sources, mode_deriv_dests, dt):
        ...
        for mode_index in xrange(len(mode_dt_sources)):
            ...
            modeDt = self.get_vec(mode_dt_sources[mode_index])
            self.put_vec((modeDt - mode)*(1./dt), mode_deriv_dests[mode_index])
    ...
    def form_A(self, A_dest, direct_mode_advanced_sources, adjoint_mode_sources, dt=None,
        ...
        for start_row_num in range(0,self.num_modes, num_rows_per_chunk):
            ...
            #a list of 'row' adjoint modes (row because Y' has rows of adjoint vecs)
            ...
            for col_num,input_source in enumerate(input_vec_sources):
                ...
                for row_num in range(start_row_num, end_row_num):
                    self.B[row_num, col_num] = \
                      self.inner_product(adjoint_modes[row_num-start_row_num], input_vec)
    ...
    def form_C(self, C_dest, output_vec_sources, direct_mode_sources, num_modes=None):
            ...
            for row_num,output_source in enumerate(output_vec_sources):
                ...
                for col_num in range(start_col_num, end_col_num):
                    self.C[row_num,col_num] = \
                        self.inner_product(output_vec, direct_modes[col_num-start_col_num])      
